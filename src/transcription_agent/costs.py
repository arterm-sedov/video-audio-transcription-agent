...
def normalize_usage(provider: str, usage: dict | None) -> Usage:
    ...
    if provider == "polza" and usage.get("cost_rub") is not None:
        rate = float(os.getenv("POLZA_RUB_TO_USD_RATE", "85"))
        if rate <= 0:
            raise ValueError("POLZA_RUB_TO_USD_RATE must be positive")
        return Usage(input_tokens, output_tokens, float(usage["cost_rub"]) / rate)
